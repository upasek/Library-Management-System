...
class Registration_S:
    def __init__(self):
        ...
        label = tkinter.Label(self.root, text='Student Registration', bg='black', fg='white')
        ...
        self.Student_ID = tkinter.Label(self.root, text='Staff ID', bg='black', fg='white')
        self.Student_ID.grid(row=4, column=1, sticky='W')
        self.Student_ID['font'] = tkinter.font.Font(size=15, family='Helvetica')
        self.Student_ID_entry = tkinter.Entry(self.root)
        self.Student_ID_entry.grid(row=4, column=1, sticky='E')
        ...
        register = tkinter.Button(self.root, text='Register', pady=10, width=20, activebackground='red',
    ...
    def checkInfo(self):
        if self.name_entry.get() and self.mobile_no_entry.get() and self.email_entry.get() and self.Student_ID_entry.get() and self.__password_entry.get():

            try:
                myc = DataBase_connector.my_db.cursor()
                sql = "INSERT INTO LibraryManagementSystem.StudentLoginInfo(student_name, mobile_no, email_ID, student_ID, s_password) VALUES(%s, %s, %s, %s, %s)"
                val = [self.name_entry.get(), self.mobile_no_entry.get(), self.email_entry.get(),
                       self.Student_ID_entry.get(), self.__password_entry.get()]
