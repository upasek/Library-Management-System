...
class update_profile:
    def __init__(self, staffID):
        ...
        sql = "SELECT * FROM LibraryManagementSystem.StaffLoginInfo WHERE staff_ID = %s"
        val = [self.val_b]
        ...
        self.Staff_ID_entry.insert(0, result[0][4])
    ...
    def update_DB(self):
        if self.name_entry.get() and self.mobile_no_entry.get() and self.email_entry.get() and self.Staff_ID_entry.get():
            self.UpdateFromDB()
        else:
            window = tkinter.Tk()
    ...
    def UpdateFromDB(self):
        ...
        sql = "UPDATE LibraryManagementSystem.StaffLoginInfo SET staff_name = %s, mobile_no = %s, email_ID = %s, staff_ID = %s WHERE (staff_ID = %s AND sr_no <> %s)"
        val = (str(self.name_entry.get()), str(self.mobile_no_entry.get()), str(self.email_entry.get()), str(self.Staff_ID_entry.get()), str(self.val_b), 0)
